...
public class APIWorkFlowSteps {
    ...
    //=======================================================================================================
    @Given("a request is prepared for updating an employee with dynamic data {string},{string} , {string}  , " +
            "{string} , {string} , {string} , {string} , {string}")
    public void a_request_is_prepared_for_updating_an_employee_with_dynamic_data(
            String empId, String firstname, String lastname, String middlename,
            String gender, String dob,
            String empStatus, String jobTitle) {
        request = given().header(APIConstants.Header_Key_Content_Type, APIConstants.Header_Value_Content_Type).
                header(APIConstants.Header_Key_Authorization, GenerateTokenSteps.token).
                body(APIPayloadConstant.updateEmployeePayLoadDaynamic
...
public void a_request_is_prepared_for_getting_an_updated_employee() {
   request=given().header(APIConstants.Header_Key_Content_Type,APIConstants.Header_Value_Content_Type).
           header(APIConstants.Header_Key_Authorization,GenerateTokenSteps.token).
           queryParam("employee_id",employee_id);
}
    @When("a GET call is made to get this updated employee")
    public void a_get_call_is_made_to_get_this_updated_employee() {
       response=request.when().get(APIConstants.GET_ONE_EMPLOYEE_URI);
    }
    @Then("the employee id {string} at the updated employee object should match with global emp id")
    public void the_employee_id_at_the_updated_employee_object_should_match_with_global_emp_id(String empId) {
       employee_id=response.jsonPath().getString(empId);
        System.out.println(employee_id);
    }
    @Then("the retrieved data at {string} object should match with the data used for updating the employee")
    public void the_retrieved_data_at_object_should_match_with_the_data_used_for_updating_the_employee
            (String employeeObject,DataTable dataTable) {
